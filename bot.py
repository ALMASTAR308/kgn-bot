...
# Campaign Constants
USER_MAX_CAP = 200.0        # Max rewards cap per user
TOTAL_POOL_CAP = 500000.0   # Total pool cap
...
async def generate_and_send_export(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    writer.writerow(["User ID", "Telegram Username", "Solana Wallet Address", "KGN Balance", "Total Referrals Count"])
    ...
    bytes_output.name = "kgn_airdrop_distribution.csv"

    total_dist = get_total_distributed()
    total_users = len(rows)

    caption = (
        "📊 **Kognitron AI ($KGN) Airdrop Report**\n\n"
        f"• **Total Registered Users:** `{total_users}`\n"
        f"• **Total Rewards Allocated:** `{total_dist:,.1f} / {TOTAL_POOL_CAP:,.0f} $KGN`\n"
        f"• **Individual Cap:** `{int(USER_MAX_CAP)} $KGN`\n\n"
        "📁 File is formatted in CSV (Ready for Excel / Solana Multisender)."
    )

    await context.bot.send_document(
        chat_id=chat_id,
        document=bytes_output,
        filename="kgn_airdrop_distribution.csv",
        caption=caption,
        parse_mode="Markdown"
    )
